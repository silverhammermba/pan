...
\end{align*}
\end{proof}
If we consider such an attack to be viable, we should make this bound as small as
possible while still allowing for extra responses. Thus we choose $t=1$, which
means the highest probability an attacker can achieve is only about
$1-1/e\approx2/3$.

% TODO show that t=1 is still pretty good
...
\begin{align*}
	...
	Q=u)P(Q=u|I=i)P(I=i)}{\sum_{i=1}^{k}P(Q=u|I=i)P(I=i)}
\end{align*}
All the terms in this formula are known. $P(I=i)$, the prior probability of
...
an accurate result (and are much simpler to implement).

% TODO diagram of different distributions
...
a certain number of queries occurring given a number of initial origins. Another
way to think of this is: assuming every peer wants the knowledge eventually,
how many queries will it take them to get it?

Consider deterministic responses as in \secref{baseline}. If we were to observe
the network over its lifetime, given $i$ origins we would see $n-i$ queries: one
for each of the peers that does not have the knowledge. Furthermore, since
queries are responded to deterministically, the order of queries is irrelevant:
it is just as easy for the $1^\text{st}$ peer to query as it is for the
$(n-i)^\text{th}$. Therefore,
\[P(Q=u|I=i)=\frac{1}{n-i-1}\]
where $u\in[1,n-i-1]$ because the prior is calculated by one of the peers in the
network, so only $n-i-1$ other peers are considered.

\section{Private Attribution Network}
