...
We note that if $r=1$ then $P(H=m)=1$ and $P(S|M=m)=P(S|H=m)$ as before. But
for a general distribution on $H$, $P(S|M=m)<P(S|H=m)$ since
$\forall k~P(S|H=k+1)<P(S|H=k)$ (by \eqnref{psk}) and \eqnref{psm} is
essentially a weighted sum over these lower-probability terms.

% TODO prove this concretely in a theorem
