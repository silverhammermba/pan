...
\begin{figure}%
    \centering
	\subfloat[$P(\Omega|I)$]{{\includegraphics[width=0.5\textwidth]{chart_omega_unrel_10_99.pdf}}}%
	\subfloat[$P(Q|I)$]{{\includegraphics[width=0.5\textwidth]{chart_qdist_unrel_10_99.pdf}}}%
	\caption{Calculating $P(Q|I)$ with unreliable peers ($r=0.99$) via network
...
\begin{figure}%
    \centering
	\subfloat[$P(\Omega|I)$]{{\includegraphics[width=0.5\textwidth]{chart_omega_exre_10_1.pdf}}}%
	\subfloat[$P(Q|I)$]{{\includegraphics[width=0.5\textwidth]{chart_qdist_exre_10_1.pdf}}}%
	\caption{Calculating $P(Q|I)$ with extra responses ($t=1$) via network
	simulation ($n=10$). The format is the same as \figref{unrelqueries}.}
...
\begin{figure}%
    \centering
	\subfloat[$P(\Omega|I)$]{{\includegraphics[width=0.5\textwidth]{chart_omega_unexre_10_1_99.pdf}}}%
	\subfloat[$P(Q|I)$]{{\includegraphics[width=0.5\textwidth]{chart_qdist_unexre_10_1_99.pdf}}}%
	\caption{Calculating $P(Q|I)$ with unreliable peers and extra responses
	($r=0.99$, $t=1$) via network simulation ($n=10$). The format is the same
...
\begin{figure}%
	...
	($n=10$). Each line represents the private leakage as a function of when
	the attacker queries. E.g. the first point represents the leakage if an
	attacker is the first peer to make a query, the second point is if they are
	the second peer, etc. The leakage is averaged over thousands of network
	simulations, so it is possible for the actual leakage to be higher or lower in a
	specific situation. Note the logarithmic scale on the $X$ axis.}
    \label{fig:compunrel}%
\end{figure}
...
this is equivalent to the unreliable peers protocol with $r=1$. Thus
\figref{compunrel} indicates that even slightly decreasing $r$ results in
a dramatic privacy benefit. Afterwards changes in $r$ are less dramatic. For
this reason we chose $r=0.99$ for all other cases where unreliable peers were
used.
